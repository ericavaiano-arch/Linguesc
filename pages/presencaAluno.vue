  ...
  <div class="min-h-screen bg-gray-50 p-8">
    ...
    <div v-else class="space-y-8">
      ...
      <p class="text-sm text-gray-500">
        ...
        <span class="ml-2 text-gray-400">· Meta de presença: {{ turma.meta_frequencia }}%</span>
      </p>

      <!-- ── CARD DE META ── -->
      <div
        class="bg-white border rounded-2xl p-6 shadow-sm"
        :class="statusMeta.cor === 'green' ? 'border-green-200' : statusMeta.cor === 'yellow' ? 'border-yellow-200' : 'border-red-200'"
      >
        <div class="flex items-start justify-between mb-4">
          ...
          <span
            ...
            :class="statusMeta.cor === 'green' ? 'text-green-600' : statusMeta.cor === 'yellow' ? 'text-yellow-600' : 'text-red-600'"
          >
            {{ percentual }}%
          </span>
        </div>
        ...
        <div class="relative w-full bg-gray-100 rounded-full h-3 mb-1">
          <div
            ...
            :class="statusMeta.cor === 'green' ? 'bg-green-500' : statusMeta.cor === 'yellow' ? 'bg-yellow-400' : 'bg-red-500'"
            :style="{ width: Math.min(percentual, 100) + '%' }"
          ></div>
          <!-- Marcador da meta -->
          <div
            class="absolute top-0 h-3 w-0.5 bg-gray-500 opacity-50"
            :style="{ left: turma.meta_frequencia + '%' }"
          ></div>
        </div>
        ...
        <div
          ...
          :class="statusMeta.cor === 'green' ? 'bg-green-50 text-green-700' : statusMeta.cor === 'yellow' ? 'bg-yellow-50 text-yellow-700' : 'bg-red-50 text-red-700'"
        >
          {{ statusMeta.mensagem }}
        </div>
      </div>
      ...
      <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <ul v-else class="space-y-3">
          ...
          >
            ...
            <span class="px-3 py-1 rounded-full text-xs font-semibold" :class="badgeClass(aula.statusAluno)">
...
const statusMeta = computed(() => {
  ...
  if (total === 0) {
    return { cor: 'green', mensagem: 'Nenhuma aula realizada ainda. Sua frequência está em dia! 🎉' }
  }
  ...
  if (freq >= meta) {
    ...
    if (faltasRestantes <= 0) {
      return { cor: 'yellow', mensagem: `Você atingiu a meta, mas não pode mais faltar para mantê-la. ⚠️` }
    }
    return {
      cor: 'green',
      mensagem: `Você está acima da meta de presença! 🎉`
    }
  } else {
