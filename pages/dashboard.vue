  ...
  <div class="min-h-screen bg-gray-50 p-8">
    ...
    <div v-else class="space-y-8">
      <div>
        ...
        >
          📅 Próximas aulas (30 dias)
        </h2>

        <div
          v-if="proximasAulas.length === 0"
          class="bg-white border border-dashed border-gray-300 rounded-2xl p-8 text-center text-gray-400 text-sm"
        >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div v-else class="space-y-5">
            <div v-for="turma in resumoPorTurma" :key="turma.id">
              <div class="flex items-center justify-between mb-1.5">
                <span
                  class="text-sm font-medium text-gray-700 truncate max-w-[60%]"
                  >{{ turma.nome }}</span
                >
                <span
                  class="text-sm font-bold"
                  :class="
                    turma.frequenciaMedia >= turma.meta
                      ? 'text-green-600'
                      : 'text-red-500'
                  "
                >
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          ...
          <ul v-else class="space-y-3 max-h-80 overflow-y-auto pr-1">
            ...
            >
              <div class="min-w-0">
                <p class="text-xs text-gray-400 truncate">{{ aluno.nome }}</p>
...
const dadosGrafico = computed(() => {
  ...
  const todasAulasRealizadas = todasAulas.value
    .filter(
      (a) =>
        a.status === "REALIZADA" &&
        turmasFiltradas.some((t) => t.id === a.turma_id),
    )
    .sort((a, b) => a.data.localeCompare(b.data));

  if (todasAulasRealizadas.length === 0) return { labels: [], datasets: [] };

  const datasets = turmasFiltradas
    .map((turma, i) => {
      const aulasDataTurma = todasAulasRealizadas.filter(
