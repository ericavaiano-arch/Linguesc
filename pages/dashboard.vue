<template>
  <div class="min-h-screen bg-gray-50 p-8">
    ...
    <!-- Header -->
    <!-- Substitua o header do dashboard -->
    <div class="mb-10 flex items-start justify-between">
      ...
      <button
        v-if="!loading"
        @click="imprimirPDF"
        class="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-300 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition no-print"
      >
        🖨️ Exportar PDF
      </button>
    </div>
    ...
    <div v-else class="space-y-10">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          ...
          <p class="text-3xl font-bold" :class="frequenciaMediaGeral >= 75 ? 'text-green-700' : 'text-red-600'">
          ...
          >
            ...
            <div class="flex items-start justify-between mb-5">
              ...
              <span
                ...
                :class="turma.frequenciaMedia >= 75
                  ? 'bg-green-100 text-green-700'
                  : turma.frequenciaMedia >= 50
            ...
            <div class="w-full bg-gray-100 rounded-full h-2 mb-5">
              <div
                ...
                :class="turma.frequenciaMedia >= 75 ? 'bg-green-500' : turma.frequenciaMedia >= 50 ? 'bg-yellow-400' : 'bg-red-500'"
...
const turmas = ref([])
const todasAulas = ref([])       // { id, turma_id, data, status }
const todasPresencas = ref([])   // { aula_id, aluno_id }
const todosVinculos = ref([])    // { turma_id, aluno_id, usuarios: { nome } }
...
onMounted(async () => {
  ...
  const [
    ...
    supabase.from('turma_aluno').select('turma_id, aluno_id, usuarios(id, nome)')
  ])
...
function alunosDaTurma(turmaId) {
  return todosVinculos.value.filter(v => v.turma_id === turmaId)
}
...
const resumoPorTurma = computed(() =>
  turmas.value.map(turma => {
    ...
    const proximaAula = todasAulas.value
      .filter(a => a.turma_id === turma.id && a.status === 'AGENDADA' && a.data >= hoje)
...
const totalAlunos = computed(() => {
  const ids = new Set(todosVinculos.value.map(v => v.aluno_id))
  return ids.size
})
...
// ─── Alunos em risco (lista detalhada) ───────────────────────────
