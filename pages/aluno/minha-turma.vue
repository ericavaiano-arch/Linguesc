    ...
    <div v-else class="space-y-6">
      ...
      <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <div class="flex items-start justify-between">
          <div>
            <h2 class="text-base font-medium text-gray-800">{{ turma.nome }}</h2>
            <p class="text-sm text-gray-500 mt-1">{{ nomeProfessor }}</p>
            <p v-if="turma.sala" class="text-sm text-gray-500 mt-0.5">Sala {{ turma.sala }}</p>
          </div>
          <span
        ...
        <div class="flex items-stretch gap-6">
          <div class="flex flex-col gap-0.5">
            ...
            <span class="text-sm font-medium text-gray-800">{{ totalAlunos }}</span>
          </div>
          <div class="w-px bg-gray-100"></div>
          <div class="flex flex-col gap-0.5">
            <span class="text-[11px] text-gray-400">Meta de frequência</span>
            <span class="text-sm font-medium text-gray-800">{{ metaFrequencia }}%</span>
          </div>
          <div class="w-px bg-gray-100"></div>
          <div class="flex flex-col gap-0.5">
...
onMounted(async () => {
  ...
  const [{ data: professorData }, { data: vinculosData }, { data: aulasData }] = await Promise.all([
    supabase.from('usuarios').select('nome').eq('id', professorId).single(),
    supabase.from('turma_aluno').select('aluno_id, usuarios(id, nome, ativo)').eq('turma_id', tId),
    supabase.from('aula').select('id, data, status').eq('turma_id', tId).order('data', { ascending: true }),
  ])
  ...
  const aulasRealizadasIds = (aulasData || []).filter(a => a.status === 'REALIZADA').map(a => a.id)
  ...
  if (alunosIds.length > 0 && aulasRealizadasIds.length > 0) {
    const [{ data: presencasData }, { data: justData }] = await Promise.all([
      supabase.from('presenca').select('aluno_id, aula_id').in('aluno_id', alunosIds).in('aula_id', aulasRealizadasIds),
      supabase.from('justificativa_falta').select('aluno_id, aula_id, status').in('aluno_id', alunosIds).in('aula_id', aulasRealizadasIds).eq('status', 'ACEITA'),
    ])
...
const totalAlunos = computed(() => todosAlunos.value.filter(a => a.ativo).length)
...
const rankingAtivos = computed(() => {
  ...
  const lista = todosAlunos.value
    ...
    .map(aluno => {
      ...
      const justAceitas = todasJustificativas.value.filter(j =>
        j.aluno_id === aluno.id &&
        !todasPresencas.value.some(p => p.aluno_id === aluno.id && p.aula_id === j.aula_id)
      ).length
      const totalValidas = presencas + justAceitas
      const frequencia = aulasRealizadas.value > 0 ? Math.round((totalValidas / aulasRealizadas.value) * 100) : 0
      return { id: aluno.id, isVoce: aluno.id === meuId, presencas: totalValidas, frequencia }
    })
    .sort((a, b) => b.frequencia - a.frequencia)

  let contador = 1
  return lista.map(aluno => ({ ...aluno, nomeAnonimo: aluno.isVoce ? 'Você' : `Colega ${contador++}` }))
})

const minhaFrequencia = computed(() => rankingAtivos.value.find(a => a.isVoce)?.frequencia ?? 0)
...
const minhaPosicao = computed(() => {
  ...
  if (freq < meta) { cor = 'red'; mensagem = 'Sua frequência está abaixo da meta. Tente não faltar!' }
  else if (pos > Math.ceil(total / 2)) { cor = 'yellow'; mensagem = 'Você está na metade inferior da turma. Dá pra melhorar!' }
