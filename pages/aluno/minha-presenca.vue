<template>
  <div class="min-h-screen bg-gray-50 p-8">
    ...
    <div v-else class="space-y-6">
      ...
      <div
        class="bg-white border rounded-2xl px-6 py-5 shadow-sm"
        :class="corBorda"
      >
        ...
        <div class="flex gap-10 mb-5">
          <div class="space-y-2.5">
            ...
            <div class="flex items-baseline gap-2">
              <span class="text-xl font-medium text-green-700">{{
                freq.totalValidas
              }}</span>
              ...
              >
                ({{ freq.justificativasAceitas }} justificada{{
                  freq.justificativasAceitas > 1 ? "s" : ""
                }})
              </span>
            </div>
            <div class="flex items-baseline gap-2">
          ...
          >
            ...
            <span class="flex items-center gap-1.5"
              ><span
                ...
                >✗</span
              >
              Falta (clique para justificar)</span
            >
          </div>
        </div>
...
const statusMeta = computed(() => {
  ...
  const totalComFuturas = total + proximasAulas.value.length;
  const maxFaltasPermitidas = Math.floor(totalComFuturas * (1 - meta / 100));
  const faltasAtuais = total - freq.value.totalValidas;
  const faltasRestantes = Math.max(0, maxFaltasPermitidas - faltasAtuais);

  if (frequencia >= meta) {
    if (faltasRestantes === 0)
      return {
        classe: "bg-yellow-50 text-yellow-700",
        mensagem: `Você atingiu a meta de presença, mas não pode mais faltar para mantê-la. ⚠️`,
      };
    return {
      classe: "bg-green-50 text-green-700",
      mensagem: `Você está acima da meta de presença! Considerando as próximas aulas, pode faltar até mais ${faltasRestantes} aula(s) e ainda manter os ${meta}%. 🎉`,
    };
  }

  const presencasNecessarias =
    Math.ceil((meta / 100) * totalComFuturas) - freq.value.totalValidas;
  const possivel = presencasNecessarias <= proximasAulas.value.length;

  if (possivel)
    return {
      classe: "bg-red-50 text-red-700",
      mensagem: `Frequência abaixo da meta de ${meta}% de presença. Você precisa comparecer em pelo menos ${presencasNecessarias} das próximas ${proximasAulas.value.length} aula(s) para recuperar.`,
    };

  return {
    classe: "bg-red-50 text-red-700",
    mensagem: `Frequência abaixo da meta de ${meta}% de presença. Não é mais possível atingir a meta mesmo comparecendo a todas as aulas restantes.`,
  };
});
...
const projecao = computed(() => {
  ...
  if (frequenciaProjetada >= meta) {
    ...
    const totalValidasAtuais = freq.value.totalValidas; // sem projetar futuras
    const presencasNecessarias =
      Math.ceil((meta / 100) * totalAulasComFuturas) - totalValidasAtuais;
    const possivel = presencasNecessarias <= presencasFuturas;

    mensagem = possivel
      ? `Abaixo da meta de ${meta}%. Precisa comparecer em pelo menos ${presencasNecessarias} das aulas restantes.`
      : `Abaixo da meta de ${meta}%. Não é mais possível atingir a meta mesmo comparecendo a todas as aulas restantes.`;
  }
