  ...
  <div class="min-h-screen bg-gray-50 p-8">
    ...
    <div v-else class="space-y-6">
      ...
      <button
        ...
        @click="$router.push('/aluno/turmas?destino=minha-presenca')"
      ...
      <div class="bg-white border rounded-2xl px-6 py-5 shadow-sm" :class="corBorda">
        ...
        <div class="flex items-start justify-between mb-5">
          <div>
            ...
            <p class="text-xs text-gray-400 mt-0.5">meta de {{ metaFrequencia }}%</p>
          </div>
          <span
            class="text-xs font-medium px-3 py-1 rounded-full"
            :class="freq.frequencia >= metaFrequencia ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'"
          >
        ...
        <div class="flex gap-8 items-start mb-5">
          <div>
            <p class="text-4xl font-medium leading-none" :class="freq.frequencia >= metaFrequencia ? 'text-green-700' : 'text-red-500'">
              {{ freq.frequencia }}%
            </p>
          ...
          <div class="flex-1">
            ...
            <div class="relative w-full bg-gray-100 rounded-full h-1.5">
              ...
              ></div>
              <div class="absolute -top-1 h-3.5 w-px bg-gray-400 rounded" :style="{ left: metaFrequencia + '%' }"></div>
            </div>
        ...
        <div class="rounded-xl px-4 py-3 text-sm font-medium flex items-start gap-2" :class="statusMeta.classe">
      ...
      <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div v-if="tabAtiva === 'historico'" class="p-0">
          <div v-if="aulasRealizadas.length === 0" class="text-center py-12 text-sm text-gray-400">
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                  <th class="text-left px-6 py-3 font-semibold text-gray-500 text-xs uppercase tracking-wide">Aluno</th>
                  <th
              ...
              <tbody>
                <tr class="hover:bg-gray-50 transition">
                  <td class="px-6 py-4 font-medium text-gray-800 whitespace-nowrap">{{ user?.nome }}</td>
                  <td v-for="aula in aulasRealizadas" :key="aula.id" class="text-center px-3 py-4">
                    <span v-if="presencasAluno.has(aula.id)" class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-green-100 text-green-700">✓</span>
                    <span v-else-if="justificativaStatus(aula.id) === 'ACEITA'" class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-blue-100 text-blue-700" title="Falta justificada e aceita">J</span>
                    <span v-else-if="justificativaStatus(aula.id) === 'PENDENTE'" class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700" title="Pendente Justificativa">⏳</span>
                    <button v-else-if="justificativaStatus(aula.id) === 'REJEITADA'" @click="abrirJustificativa(aula)" class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-red-100 text-red-600 hover:bg-red-200 transition" title="Rejeitada — clique para reenviar">✗</button>
                    <button v-else @click="abrirJustificativa(aula)" class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-red-100 text-red-600 hover:bg-red-200 transition" title="Falta — clique para justificar">✗</button>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="px-6 py-3 border-t border-gray-50 flex items-center flex-wrap gap-4 text-xs text-gray-400">
            <span class="flex items-center gap-1.5"><span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-green-100 text-green-700 font-bold text-xs">✓</span> Presente</span>
            <span class="flex items-center gap-1.5"><span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-blue-100 text-blue-700 font-bold text-xs">J</span> Justificada</span>
            <span class="flex items-center gap-1.5"><span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-yellow-100 text-yellow-700 font-bold text-xs">⏳</span> Pendente Justificativa</span>
            <span class="flex items-center gap-1.5"><span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-red-100 text-red-600 font-bold text-xs">✗</span> Falta (clique para justificar)</span>
          </div>
        </div>

        <!-- ── TAB: Projeção ── -->
        <div v-if="tabAtiva === 'projecao'" class="p-6 space-y-5">
          <div v-if="proximasAulas.length === 0" class="text-sm text-gray-400 text-center py-6">
            Nenhuma aula agendada para simular.
          </div>

          <div v-else class="space-y-5">
            <p class="text-sm text-gray-400">Toque em cada aula para simular presença ou falta.</p>

            <div v-if="aulasRealizadas.length > 0" class="space-y-2 mb-4">
              <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">Aulas realizadas</p>
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                ...
                >
                  <span class="text-lg mb-1">{{ presencasAluno.has(aula.id) ? '✓' : '✗' }}</span>
                  <span class="text-xs font-semibold">{{ formatarDataCurta(aula.data) }}</span>
                  <span class="text-xs mt-0.5 opacity-70">
                    {{ presencasAluno.has(aula.id) ? 'Presente' : justificativaStatus(aula.id) === 'ACEITA' ? 'Justificada' : 'Falta' }}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
              <button
                ...
                :class="aulasSimuladasFalta.has(aula.id) ? 'border-red-300 bg-red-50 text-red-600' : 'border-green-300 bg-green-50 text-green-700'"
              >
                <span class="text-lg mb-1">{{ aulasSimuladasFalta.has(aula.id) ? '✗' : '✓' }}</span>
                <span class="text-xs font-semibold">{{ formatarDataCurta(aula.data) }}</span>
                <span class="text-xs mt-0.5 opacity-70">{{ aulasSimuladasFalta.has(aula.id) ? 'Falta' : 'Presente' }}</span>
              </button>
            </div>

            <div
              class="rounded-xl px-5 py-4 border transition-all"
              :class="projecao.frequencia >= (turma?.meta_frequencia ?? 75) ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'"
            >
              <div class="flex items-center justify-between mb-3">
                <span class="text-sm font-medium text-gray-600">Frequência projetada</span>
                <span class="text-2xl font-bold" :class="projecao.frequencia >= (turma?.meta_frequencia ?? 75) ? 'text-green-700' : 'text-red-600'">
                  {{ projecao.frequencia }}%
                </span>
              </div>
              <div class="relative w-full bg-gray-100 rounded-full h-2 mb-3">
                <div
                  ...
                  :class="projecao.frequencia >= (turma?.meta_frequencia ?? 75) ? 'bg-green-500' : 'bg-red-400'"
                  :style="{ width: Math.min(projecao.frequencia, 100) + '%' }"
                ></div>
                <div class="absolute top-0 h-2 w-0.5 bg-gray-400" :style="{ left: (turma?.meta_frequencia ?? 75) + '%' }"></div>
              </div>
              <p class="text-sm font-medium" :class="projecao.frequencia >= (turma?.meta_frequencia ?? 75) ? 'text-green-700' : 'text-red-600'">
                {{ projecao.mensagem }}
              </p>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 text-center">
              <div class="bg-gray-50 rounded-xl p-3">
                ...
                <p class="font-bold text-gray-700">{{ proximasAulas.length }}</p>
              </div>
              <div class="bg-gray-50 rounded-xl p-3">
                <p class="text-xs text-gray-400 mb-1">Faltas simuladas</p>
                <p class="font-bold text-red-500">{{ aulasSimuladasFalta.size }}</p>
              </div>
              <div class="bg-gray-50 rounded-xl p-3">
                <p class="text-xs text-gray-400 mb-1">Pode ainda faltar</p>
                <p class="font-bold" :class="projecao.faltasRestantes > 0 ? 'text-green-600' : 'text-red-500'">
                  {{ projecao.faltasRestantes }}
                </p>
              </div>
            </div>

            <button v-if="aulasSimuladasFalta.size > 0" @click="aulasSimuladasFalta = new Set()" class="text-xs text-gray-400 hover:text-gray-600 transition underline">
              Resetar simulação
            </button>
          </div>
        </div>
    ...
    <Transition name="fade">
      <div v-if="modalAberto" class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
        <div class="bg-white rounded-2xl p-6 shadow-xl w-full max-w-md">
          <h3 class="text-lg font-semibold text-gray-800 mb-1">📝 Justificar Falta</h3>
          <p class="text-sm text-gray-400 mb-4">Aula de {{ aulaSelecionada ? formatarDataCompleta(aulaSelecionada.data) : '' }}</p>

          <div v-if="justificativaExistente?.status === 'REJEITADA' && justificativaExistente?.resposta" class="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4">
            <p class="text-xs font-semibold text-red-600 mb-1">Motivo da rejeição:</p>
            <p class="text-sm text-red-700">{{ justificativaExistente.resposta }}</p>
          </div>
          ...
          ></textarea>
          <p class="text-xs text-gray-400 mt-1 text-right">{{ textoJustificativa.length }}/500</p>

          <div class="flex gap-3 mt-4">
            <button @click="fecharModal" class="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition">Cancelar</button>
            <button
...
onMounted(async () => {
  ...
  if (!vinculos || vinculos.length === 0) { loading.value = false; return }
  ...
  if (turmaId) {
    ...
    router.replace('/aluno/turmas?destino=minha-presenca')
    return
  }
  ...
  const [{ data: presencasData }, { data: justificativasData }] = await Promise.all([
    ...
    supabase.from('justificativa_falta').select('aula_id, status, resposta, texto').eq('aluno_id', alunoId),
  ])
...
const aulasRealizadas = computed(() => aulas.value.filter(a => a.status === 'REALIZADA'))
...
const freq = computed(() => {
  ...
  const alunoId = user.value?.id ?? ''
  const presencasArr = [...presencasAluno.value].map(aula_id => ({ aluno_id: alunoId, aula_id }))
  const justAceitas = justificativas.value.filter(j => j.status === 'ACEITA').map(j => ({ aluno_id: alunoId, aula_id: j.aula_id }))
  return calcularFrequenciaSync(alunoId, aulasIds, presencasArr, justAceitas)
})

const totalFaltas = computed(() =>
  aulasRealizadas.value.filter(a => !presencasAluno.value.has(a.id) && justificativaStatus(a.id) !== 'ACEITA').length
)

const statusOk = computed(() => statusMeta.value.classe.includes('green'))
...
const statusMeta = computed(() => {
  ...
  if (total === 0) return { classe: 'bg-green-50 text-green-700', mensagem: 'Nenhuma aula realizada ainda. 🎉' }

  if (frequencia >= meta) {
    const maxFaltas = Math.floor(total * (1 - meta / 100))
    const faltasRestantes = maxFaltas - totalFaltas.value
    if (faltasRestantes <= 0) return { classe: 'bg-yellow-50 text-yellow-700', mensagem: 'Você atingiu a meta, mas não pode mais faltar para mantê-la. ⚠️' }
    return { classe: 'bg-green-50 text-green-700', mensagem: `Você está acima da meta! Pode faltar até mais ${faltasRestantes} aula(s) e ainda manter os ${meta}%. 🎉` }
  }

  const presencasNecessarias = Math.ceil((meta / 100) * total) - freq.value.totalValidas
  return { classe: 'bg-red-50 text-red-700', mensagem: `Frequência abaixo da meta de ${meta}%. Você precisaria ter comparecido em mais ${presencasNecessarias} aula(s) das já realizadas.` }
})
...
const projecao = computed(() => {
  ...
  if (totalAulasComFuturas === 0) return { frequencia: 0, mensagem: '', faltasRestantes: 0 }

  const frequenciaProjetada = Math.round((totalValidas / totalAulasComFuturas) * 100)
  const maxFaltasPermitidas = Math.floor(totalAulasComFuturas * (1 - meta / 100))
  ...
  if (frequenciaProjetada >= meta) {
    mensagem = faltasRestantes === 0
      ? `Acima da meta. Não pode mais faltar.`
      : `Acima da meta. Pode faltar mais ${faltasRestantes} aula(s) e manter os ${meta}%.`
  } else {
    const presencasNecessarias = Math.ceil((meta / 100) * totalAulasComFuturas) - totalValidas
    mensagem = presencasNecessarias <= 0
  ...
  return { frequencia: Math.max(0, frequenciaProjetada), mensagem, faltasRestantes }
})
...
const justificativaExistente = computed(() =>
  justificativas.value.find(j => j.aula_id === aulaSelecionada.value?.id) ?? null
)

function abrirJustificativa(aula) {
  aulaSelecionada.value = aula
  const existente = justificativas.value.find(j => j.aula_id === aula.id)
  textoJustificativa.value = existente?.status === 'REJEITADA' ? existente.texto : ''
...
async function enviarJustificativa() {
  ...
  try {
    if (existente) {
      const { error } = await supabase.from('justificativa_falta').update({
      ...
      justificativas.value = justificativas.value.map(j =>
        j.aula_id === aulaId ? { ...j, texto: textoJustificativa.value.trim(), status: 'PENDENTE', resposta: null } : j
      )
    } else {
      ...
      if (error) throw error
      justificativas.value.push({ aula_id: aulaId, status: 'PENDENTE', resposta: null, texto: textoJustificativa.value.trim() })
    }
