<template>
  <div class="min-h-screen bg-gray-50 p-8">
    <div class="mb-10 flex items-start justify-between">
      <div>
        ...
        <p class="text-gray-500 mt-2">
          Visão geral de todas as turmas e professores.
        </p>
    ...
    <!-- Filtros -->
    <div
      class="bg-white border border-gray-200 rounded-2xl p-4 mb-8 flex flex-wrap items-center gap-4"
    >
        ...
        <div class="flex flex-col md:flex-row">
          ...
          >
            ...
            <span
              ...
              :class="
                frequenciaGeralMedia >= metaFrequencia
                  ? 'text-green-700'
                  : 'text-red-600'
              "
          ...
          <div class="flex-1 flex flex-col gap-1 px-6 py-5">
            ...
            <span
              class="text-3xl font-semibold"
              :class="
                alunosEmRisco.length > 0 ? 'text-red-600' : 'text-green-700'
              "
            >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div v-for="turma in resumoPorTurma" :key="turma.id">
              <div class="flex items-center justify-between mb-1.5">
                ...
                <span
                  ...
                  :class="
                    turma.frequenciaMedia >= metaFrequencia
                      ? 'text-green-600'
                      : 'text-red-500'
                  "
                >
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h2
            class="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-6"
          >
            ⚠️ Frequência abaixo da meta
          </h2>

          <div
            v-if="alunosEmRisco.length === 0"
            class="text-sm text-gray-400 text-center py-8"
          >
          ...
          <ul v-else class="space-y-3 max-h-80 overflow-y-auto pr-1">
            ...
            >
              <div class="min-w-0">
                ...
                <p v-if="aluno.email" class="text-xs text-gray-400 truncate">
                  {{ aluno.email }}
                </p>
              ...
              <div class="flex-shrink-0 ml-3 text-right">
                ...
                <p class="text-xs text-gray-400">
                  {{ aluno.presencas }}/{{ aluno.aulasRealizadas }}
                </p>
              </div>
            </li>
          </ul>
      ...
      <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
          ...
          >
            ...
            <option v-for="t in turmasFiltradas" :key="t.id" :value="t.id">
              {{ t.nome }}
            </option>
          </select>
        </div>
...
onMounted(async () => {
  await Promise.all([
    ...
    (async () => {
      ...
      professores.value =
        professoresData?.map((d) => d.usuarios).filter(Boolean) || [];
...
const turmasFiltradas = computed(() => {
  return todasTurmas.value.filter((t) => {
    if (professorFiltro.value && t.professor_id !== professorFiltro.value)
      return false;
...
const turmasFiltradasIds = computed(
  () => new Set(turmasFiltradas.value.map((t) => t.id)),
);
...
function aulasRealizadasDaTurma(turmaId) {
  return todasAulas.value.filter(
    (a) => a.turma_id === turmaId && a.status === "REALIZADA",
  );
}

function alunosAtivosDaTurma(turmaId) {
  return todosVinculos.value.filter(
    (v) => v.turma_id === turmaId && v.usuarios?.ativo !== false,
  );
}
...
const dadosGrafico = computed(() => {
  const turmasParaGrafico =
    turmaSelecionadaGrafico.value === "todas"
      ? turmasFiltradas.value
      : turmasFiltradas.value.filter(
          (t) => t.id === turmaSelecionadaGrafico.value,
        );
  ...
  const aulasRealizadas = todasAulas.value
    .filter(
      (a) =>
        a.status === "REALIZADA" &&
        turmasParaGrafico.some((t) => t.id === a.turma_id),
    )
    .sort((a, b) => a.data.localeCompare(b.data));

  if (aulasRealizadas.length === 0) return { labels: [], datasets: [] };

  const datasets = turmasParaGrafico
    .map((turma, i) => {
      const aulasDaTurma = aulasRealizadas.filter(
      ...
      const dados = aulasDaTurma.map((_, idx) => {
        ...
        alunos.forEach((v) => {
          const freq = calcularFrequenciaSync(
            v.aluno_id,
            aulasAte,
            todasPresencas.value.map((p) => ({
              aluno_id: p.aluno_id,
              aula_id: p.aula_id,
            })),
            todasJustificativas.value.map((j) => ({
              aluno_id: j.aluno_id,
              aula_id: j.aula_id,
            })),
          );
